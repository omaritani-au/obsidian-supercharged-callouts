...
const STYLE_ID = 'custom-callout-styles';
function hexToRgb(hex: string): string | null { const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex); return result ? `${parseInt(result[1], 16)}, ${parseInt(result[2], 16)}, ${parseInt(result[3], 16)}` : null; }

export default class SuperchargedCalloutsPlugin extends Plugin {
    settings: MyPluginSettings;

    async onload() {
        console.log("Loading Supercharged Callouts Plugin");
        await this.loadSettings();
        this.addSettingTab(new CalloutManagerSettingTab(this.app, this));
        this.applyPluginStyles();

        this.registerEvent(this.app.workspace.on('editor-menu', (menu: Menu, editor: Editor) => { menu.addItem((item: MenuItem) => { item.setTitle("Insert Supercharged Callout...").setIcon("layout-template").onClick(() => new AdvancedCalloutModal(this.app, editor, this).open()); }); }));

        this.registerMarkdownPostProcessor((element, context) => {
            const multiColumnContainers = element.findAll('.callout[data-callout="multi-column"]');
            if (multiColumnContainers.length === 0) return;

            multiColumnContainers.forEach(containerEl => {
                containerEl.addClass('mcm-container');
                const contentEl = containerEl.querySelector<HTMLElement>(':scope > .callout-content');
                if (contentEl) {
    ...
    onunload() { this.removePluginStyles(); }
    async loadSettings() { this.settings = Object.assign({}, DEFAULT_SETTINGS, await this.loadData()); }
    async saveSettings() { await this.saveData(this.settings); }
    public applyPluginStyles() {
        this.removePluginStyles();
        if (this.settings.calloutStyle !== 'default') { document.body.classList.add(`callouts-style-${this.settings.calloutStyle}`); }
    ...
    private generateCssFromSettings(): string {
        ...
        this.settings.customCallouts.forEach(callout => {
            ...
            if (rgbColor) { const iconName = callout.icon.startsWith('lucide-') ? callout.icon : `lucide-${callout.icon}`; cssContent += `.callout[data-callout="${callout.name}"] { --callout-color: ${rgbColor}; --callout-icon: ${iconName}; }\n`; }
        });
